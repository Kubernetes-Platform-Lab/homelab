...
| Impact Level | High |
| Context | We want to provide persistent block/filesystem storage via democratic-csi. Our storage hardware is an external server that already hosts a ZFS pool. The goal is to expose that pool to k8s reliably using democratic-csi, while keeping k8s nodes stateless and avoiding using local k8s nodes storage. We wanted to server all storage via iSCSI |
...
| Consequences | **Pros:** Reuse of existing ZFS server to manage PVs; Centralized strorage lifecycle management (snapshots, expanding ZFS pool); Storage is completely isolated and independent from k8s nodes - this way it can be used by multiple clusters if we plan to add more in the future. **Cons:** Unfortunately during implementation it turned out that ZFS over iSCSI can be used to serve block PV's but it is not possible to create filesystem PV's reliably with such setup; We have also notices problems with PV removal as they were removed from k8s but still existed on ZFS server as "zombie" volumes |
