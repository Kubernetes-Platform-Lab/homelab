...
| Alternatives | <li>OpenEBS - considered: complex but allows to create both filesystem and block PVs</li><li>RustFS - rejected: low maturity and not really battle-tested</li><li>SeaweedFS - considered: good scalability but cannot serve block PVs</li> |
| Pros & Cons | **Pros**<ul><li>Reuse of existing ZFS server to manage PVs</li><li>Centralized strorage lifecycle management (snapshots, expanding ZFS pool)</li><li>Storage is completely isolated and independent from k8s nodes - this way it can be used by multiple clusters if we plan to add more in the future</li></ul>**Cons**<ul><li>Unfortunately during implementation it turned out that ZFS over iSCSI can be used to serve block PV's but it is not possible to create filesystem PV's reliably with such setup</li></ul> |
